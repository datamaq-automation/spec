# SRS-SPECS: {nombre_del_sistema_o_proyecto} — SSOT & Especificación Fullstack SSR (FastAPI + Jinja2)

> **Documento:** `srs-spec-jinja2-ssr.md`  
...
* **Arquitectura:** Clean Architecture Canónica (4 Círculos de Uncle Bob) + Screaming DDD + Server-Side Rendering (SSR).
* **Framework Web:** FastAPI (asíncrono, OpenAPI autodocumentado, montaje de static y routers).
* **Motor de Renderizado:** Jinja2 (`Jinja2Templates`, herencia de plantillas `base.html`, bloques y partials).
* **Formularios & Cargas:** `python-multipart` para parseo seguro de datos de formulario HTTP POST.
...
* **Testing:** Pytest (`pytest-asyncio`, `httpx`, validador de sintaxis y balance de templates).
* **Linters & Tipado:** Ruff y Pyright (modo estricto).

### 4.2. Estructura Canónica de Directorios (FastAPI + Jinja2 Monolith SSR)
...
│
├── templates/                                   # Capa de Presentación SSR (Jinja2)
│   ├── base.html                                # Layout raíz maestro con bloques head, content, scripts
│   ├── index.html                               # Vistas completas por página
│   └── partials/                                # Componentes HTML reutilizables (header, footer, nav)
│       ├── header.html
│       └── footer.html
│
├── static/                                      # Archivos estáticos servidos por FastAPI (/static)
│   ├── css/
│   │   └── styles.css                           # Hojas de estilos compiladas o nativas
│   ├── js/                                      # Scripts de comportamiento de cliente ligero
│   └── img/                                     # Favicons, logos e imágenes estáticas
│
├── src/
...
│   │       ├── entities.py                      # Entidades de negocio con identidad
...
│   │
│   ├── application/                             # CÍRCULO 2: Reglas de la Aplicación (Casos de Uso)
│   │   └── {bounded_context_tematico}/
│   │       ├── __init__.py                      # (0 bytes obligatorio)
│   │       ├── use_cases/                       # Orquestación de Casos de Uso
│   │       ├── dtos/                            # Data Transfer Objects
│   │       └── mappers/                         # Traductores bidireccionales DTO <-> Entity
│   │
│   ├── adapters/                                # CÍRCULO 3: Interface Adapters
│   │   └── {bounded_context_tematico}/
│   │       ├── __init__.py                      # (0 bytes obligatorio)
│   │       ├── controllers/                     # Controladores que reciben DTOs y llaman al UseCase
│   │       ├── presenters/                      # Formatean ResponseDTO a context Jinja2 / JSON
│   │       └── gateways/                        # Adaptadores hacia servicios externos
│   │
│   ├── infrastructure/                          # CÍRCULO 4: Frameworks & Drivers
│   │   ├── fastapi/                             # Mecanismo de entrega Web
│   │   │   ├── routers/                         # Routers HTML (SSR) y REST (API JSON)
│   │   │   │   ├── __init__.py
│   │   │   │   └── web.py                       # Rutas de renderizado Jinja2TemplateResponse
│   │   │   ├── templates.py                     # Instanciador centralizado Jinja2Templates
...
│   │       ├── __init__.py                      # (0 bytes obligatorio)
│   │       ├── config.py                        # Settings con TEMPLATES_DIR y STATIC_DIR
│   │       └── logger.py                        # Logging estructurado
│   │
│   └── main.py                                  # Montaje ASGI (/static, routers, lifespan)
│
├── scripts/
│   └── validate_templates.py                    # Validador estático de balance de CSS y Jinja en style
│
└── tests/
    ├── __init__.py                              # (0 bytes obligatorio)
    ├── test_architecture.py                     # Validador AST del Guantelete de Restricciones
    ├── test_god_components.py                   # Auditor de Componentes Monolíticos
    ├── test_clean_design.py                     # Auditor de Código Muerto y Sobreingeniería
    ├── test_validate_templates.py               # Tests unitarios del validador de plantillas HTML
    ├── unit/                                    # Pruebas unitarias de domain y use_cases
    ├── integration/                             # Pruebas de integración con DB/gateways
    └── e2e/                                     # Pruebas E2E de vistas HTML y endpoints REST
...
### 5.2. Las 12 Baterías del Guantelete (`tests/test_architecture.py` & `scripts/validate_templates.py`)
...
12. **`validate_templates`:** Valida estáticamente los templates Jinja2 (prohíbe lógica en atributos `style="..."` e identifica llaves desbalanceadas en bloques `<style>`).
...
# 5. Validación Estática de Plantillas HTML / Jinja2
python3 scripts/validate_templates.py

# 6. Suite Completa de Pruebas en Pytest (Unit, Integration, E2E y Templates)
